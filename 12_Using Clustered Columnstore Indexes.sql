...
-- Four new tables have been added to the AdventureWorks2014 database
-- dbo.bigTransactionHistory				- Uses Row store with no data compression
-- dbo.bigTransactionHistoryRowCompressed   - Uses Row store ROW data compression
...
-- Force a clustered index scan from row store with ROW compression
SELECT COUNT(*) AS [Row Count]
FROM dbo.bigTransactionHistoryRowCompressed WITH (INDEX(0));
...
-- Query 1 Elapsed time 4246ms, 45% of batch cost
...
-- Query 2 Elapsed time 2893ms, 31% of batch cost  
-- Table 'bigTransactionHistoryRowCompressed'. Scan count 9, logical reads 185797, physical reads 1, read-ahead reads 185249, lob logical reads 0, lob physical reads 0, lob read-ahead reads 0.


-- Query 3 Elapsed time 2120ms, 22% of batch cost  
...
-- Query 4 Elapsed time 196ms, 2% of batch cost
