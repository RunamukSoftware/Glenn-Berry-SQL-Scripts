...
These tables use ROW data compression

dbo.bigProductRowCompressed
dbo.bigTransactionHistoryRowCompressed
...
ALTER TABLE bigProductRowCompressed
ALTER COLUMN ProductId INT NOT NULL	
GO

ALTER TABLE bigProductRowCompressed
ADD CONSTRAINT pk_bigProductRowCompressed PRIMARY KEY (ProductId) WITH (DATA_COMPRESSION = ROW);
GO
...
INTO bigTransactionHistoryRowCompressed
FROM
(
	SELECT
		p.ProductID, 
		...
		CASE
			...
			ELSE 1 
		END AS ProductGroup
	FROM bigproductRowCompressed p
) AS p1
CROSS APPLY
(
	SELECT
...
ALTER TABLE bigTransactionHistoryRowCompressed
ALTER COLUMN TransactionID INT NOT NULL
GO


ALTER TABLE bigTransactionHistoryRowCompressed
ADD CONSTRAINT pk_bigTransactionHistoryRowCompressed PRIMARY KEY (TransactionID) WITH (DATA_COMPRESSION = ROW);
GO


CREATE NONCLUSTERED INDEX IX_ProductId_TransactionDate
ON bigTransactionHistoryRowCompressed
(
	ProductId,
	TransactionDate
)
